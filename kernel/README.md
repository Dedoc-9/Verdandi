...
| `fast.rs` | GAUNTLET-1: a SIBLING of `emit` (never `mantle.rs`), the optimization staircase — each tread byte-identical to the frozen emit. `emit` = GAUNTLET-1c's **row-major floor DDA** (ceiling + wall an exact transcription; the floor's perspective divide done per-ROW by a Bresenham-exact recurrence, O(rows) not O(pixels)). `emit_collapse` = GAUNTLET-1b's **divide-collapse** (2 `div_euclid`/floor px), kept verbatim as the same-apparatus speed BASELINE. The differential harness (`--fast`, rows `gauntlet1-equiv` / `gauntlet1b-reduction` / `gauntlet1c-dda`) proves both byte-identical to the frozen emit and gates the deterministic per-row reduction before any speed claim |
